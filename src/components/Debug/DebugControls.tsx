...
export interface OceanControls {
  waveAmp: number; waveSpeed: number
  waterScale: number; cellSmoothness: number; edgeThreshold: number
  edgeSoftness: number; cellSpeed: number
  flowX: number; flowZ: number; noiseScale: number
  noiseFlowSpeed: number; distortAmount: number
  deepColor: string; midColor: string; midPos: number
  highlightColor: string; opacity: number; deepOpacity: number
  fresnelPower: number; fresnelStrength: number
  foamAmount: number
}

export interface ShipControls {
  moveSpeed: number; turnSpeed: number
  baseY: number; bobAmp: number; bobSpeed: number
  tiltMax: number; tiltSpeed: number
  partLife: number; partSpeed: number
}
...
export function DebugProvider({ children }: { children: ReactNode }) {
  ...
  }, { collapsed: true }) as OceanControls

  const ship = useControls('Ship', {
    Movement: folder({
  ...
  }, { collapsed: true }) as ShipControls

  return (
    <DebugContext.Provider value={{ ocean, ship }}>
      {children}
