export const VERT = /* glsl */ `
  ...
  void main() {
    ...
    // Primary swells — dir is the negative of movement direction (crests travel in -dir)
    // Target movement: world XZ (+0.966, -0.258) = screen 2→8 o'clock
    w = waveContrib(xz, vec2(-0.97,  0.26), 0.08, 0.40, 0.00, 1.2 * uWaveSpeed); y += w.x; dHdx += w.y; dHdz += w.z;
    w = waveContrib(xz, vec2(-0.87,  0.50), 0.13, 0.25, 1.57, 0.8 * uWaveSpeed); y += w.x; dHdx += w.y; dHdz += w.z;
    w = waveContrib(xz, vec2(-0.97,  0.00), 0.22, 0.12, 3.14, 1.5 * uWaveSpeed); y += w.x; dHdx += w.y; dHdz += w.z;
    w = waveContrib(xz, vec2(-0.50,  0.87), 0.35, 0.08, 0.78, 2.0 * uWaveSpeed); y += w.x; dHdx += w.y; dHdz += w.z;
    w = waveContrib(xz, vec2(-0.26, -0.97), 0.50, 0.04, 2.30, 2.8 * uWaveSpeed); y += w.x; dHdx += w.y; dHdz += w.z;
