...
const CORNERS = [[-0.5, 0], [0.5, 0], [-0.5, 1], [0.5, 1]] as const
...
export default function Rain({ shipRef }: { shipRef: React.RefObject<THREE.Group | null> }) {
  const { geo, mat } = useMemo(() => {
    ...
    for (let i = 0; i < RAIN_COUNT; i++) {
      const xs = Math.random(), ps = Math.random(), ds = Math.random()
      for (let v = 0; v < 4; v++) {
        const vi = i * 4 + v
        seeds[vi * 3]     = xs; seeds[vi * 3 + 1] = ps; seeds[vi * 3 + 2] = ds
        corners[vi * 2]   = CORNERS[v][0]; corners[vi * 2 + 1] = CORNERS[v][1]
      }
      const ii = i * 6, vi = i * 4
